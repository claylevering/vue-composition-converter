...
export const getMethodExpression = (node: ts.Node, sourceFile: ts.SourceFile): ConvertedExpression[] => {
    if (findDescendantArrowFunction(node)) throw new Error('Arrow Functions not allowed as root methods.');
    if (ts.isMethodDeclaration(node)) {
        ...
        body.split('\n').forEach((line) => {
            if (isVariableAssignment(line)) {
                throw new Error(`property ${name} is assigned to itself. This is not allowed.`);
            }
        });
