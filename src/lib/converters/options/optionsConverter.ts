...
import { ConvertedExpression, getNodeByKind, lifecycleNameMap, objToString } from '../../helper';
...
const internalConvertOptions = (exportObject: ts.ObjectLiteralExpression, sourceFile: ts.SourceFile) => {
                ...
                props.properties.forEach((prop) => {
                    if (ts.isPropertyAssignment(prop)) {
                        ...
                        const typeProperty = propObject.properties.find(
                            (p) => ts.isPropertyAssignment(p) && p.name.getText(sourceFile) === 'type'
                        ) as ts.PropertyAssignment;
                        const defaultProperty = propObject.properties.find(
                            (p) => ts.isPropertyAssignment(p) && p.name.getText(sourceFile) === 'default'
                        ) as ts.PropertyAssignment;
                        const requiredProperty = propObject.properties.find(
                            (p) => ts.isPropertyAssignment(p) && p.name.getText(sourceFile) === 'required'
                        ) as ts.PropertyAssignment;

                        const propRequired = requiredProperty?.initializer.getText(sourceFile) || 'false';
                        const propName = propRequired === 'true' ? prop.name.getText(sourceFile) : `${prop.name.getText(sourceFile)}?`;
                        ...
                        if (ts.isAsExpression(typeProperty.initializer)) {
                            ...
                            const propType = typeArg.getText(sourceFile).replace('PropType<', '').slice(0, -1);
    ...
    const propsRefPropsSuperDefaults = {
        use: 'props',
        expression: `const props = withDefaults(defineProps<Props>(), ${objToString(propDefaults)}`,
        returnNames: ['props'],
        pkg: 'ignore'
    };

    const propsRefPropsDefaults = [
        {
            use: 'props',
        ...
        Object.keys(propDefaults).length === 0
            ? { use: 'props', expression: `const props = defineProps<Props>()`, returnNames: ['props'], pkg: 'ignore' }
            : propsRefPropsSuperDefaults
    ];

    const propsRefProps: ConvertedExpression[] = propNames.length === 0 ? [] : propsRefPropsDefaults;
...
function convertPropType(type: string) {
    return type.replace('String', 'string').replace('Number', 'number').replace('Boolean', 'boolean');
}
...
const convertOptions = (sourceFile: ts.SourceFile) => {
    ...
    if (exportAssignNode) {
        const objectNode = getNodeByKind(exportAssignNode, ts.SyntaxKind.ObjectLiteralExpression);
        if (objectNode && ts.isObjectLiteralExpression(objectNode)) {
            return internalConvertOptions(objectNode, sourceFile);
        }
    }
    return null;
};
