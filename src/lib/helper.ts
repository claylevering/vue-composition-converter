...
export const getNodeByKind = (node: ts.Node, kind: ts.SyntaxKind): ts.Node | undefined => {
    const find = (noder: ts.Node): ts.Node | undefined =>
        ts.forEachChild(noder, (child) => {
            if (child.kind === kind) {
                return child;
            }
            return find(child);
...
export const getInitializerProps = (node: ts.Node): ts.ObjectLiteralElementLike[] => {
...
export const contextProps = ['attrs', 'slots', 'parent', 'root', 'listeners', 'refs', 'emit'];
...
export const replaceThisContext = (str: string, refNameMap: Map<string, true>, propNameMap: Map<string, true>) =>
    str
...
export const getImportStatement = (setupProps: ConvertedExpression[]) => {
    ...
    if (extraImports.length)
        results.push(...ts.createSourceFile('', `import { ${extraImports.join(',')} } from 'pinia'`, ts.ScriptTarget.Latest).statements);

    return results;
};

export const getExportStatement = (setupProps: ConvertedExpression[], propNames: string[], otherProps: ConvertedExpression[]) => {
    const body = ts.factory.createBlock(getSetupStatements(setupProps));

    if (otherProps.length) {
        body.statements.push(ts.factory.createIdentifier('\n//TODO: 対応していないプロパティです。手動で書き換えてください。:\n'));
...
export const sortMap = ['emitter', 'props', 'ref', 'store', 'storeToRefs', 'computed'];

export const findEmiters = (str: string, emitterSet: Set<string>) =>
    str.replace(/this\.\$(\w+)/g, (_, p1) => {
        if (p1 === 'emit') {
...
export const getSetupStatements = (setupProps: ConvertedExpression[]) => {
    ...
    setupProps.forEach(({ expression }) => findEmiters(expression, emitterNameSet));

    if (emitterNameSet.size !== 0)
        setupProps.unshift({
            use: 'emitter',
            expression: `const emit = defineEmits(${JSON.stringify(Array.from(emitterNameSet))})`
        });
    ...
    setupProps.forEach((val) => {
        ...
        if (returnNames != null && use != null && /^(toRefs|ref|computed|storeToRefs)$/.test(use)) {
    ...
    return [...setupProps, { expression: returnPropsStatement }]
        .sort((a, b): number => {
            if (typeof b.sort === 'number' && typeof a.sort === 'number') return a.sort - b.sort;
            return 0;
        })

        .filter((value, index, self) => index === self.findIndex((t) => t.expression === value.expression))
        .reduce((pv: ConvertedExpression[], cv: ConvertedExpression) => {
            const previous = pv[pv.length - 1];
            if ((previous && cv.sort !== previous.sort) || cv.use === 'computed' || !cv.use) pv.push({ isBreak: true, expression: '' });
            pv.push(cv);
            return pv;
        }, [])
        .map(({ expression, isBreak }) => {
            ...
            return ts.createSourceFile('', replaceThisContext(expression, refNameMap, propNameMap), ts.ScriptTarget.Latest).statements;
        })
        .flat();
};
...
export function throwErrorOnDestructuring(string: string) {
    ...
    if (bodyIncludesThis) {
        throw new Error(`destructuring 'this' in ${name}, ${bodyIncludesThis}could be a conflict. `);
    }
}
